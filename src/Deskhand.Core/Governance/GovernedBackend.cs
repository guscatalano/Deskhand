...
public sealed class GovernedBackend(IAutomationBackend inner, ControlState state, AuditLog audit, ICaptureNotifier? notifier = null, MacroRecorder? recorder = null) : IAutomationBackend
{
    ...
    public void Drag(int fromX, int fromY, int toX, int toY, string button, int steps, int holdMs) { RequireInput("drag"); Audited<object?>("drag", $"{fromX},{fromY}→{toX},{toY} {button}", () => { inner.Drag(fromX, fromY, toX, toY, button, steps, holdMs); return null; }); recorder?.RecordInput(FleetMethods.Drag, new { fromX, fromY, toX, toY, button, steps, holdMs }); }
