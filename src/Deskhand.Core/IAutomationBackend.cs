...
public interface IAutomationBackend : IDisposable
{
    ...
    /// <summary>Every running process with the top-level windows it owns (empty for background
    /// processes). The windows carry live refs, so each can be expanded straight into the UIA tree.</summary>
    IReadOnlyList<ProcessInfoDto> GetProcesses();
